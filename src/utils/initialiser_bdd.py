...
if False:
    ...
    u_sql.renommer_toutes_colonnes_toutes_tables()

    # 4) Ajouter une clef primaire (pk) à toutes les tables
    u_sql.migrate_add_pk()

    # 5) Supprimer colonne "id" de toutes les tables
    u_sql.supprimer_colonne_toutes_tables("id")

    # 6) Renommer la colonne "pk" de toutes les tables en "id"
    u_sql.renommer_colonne_pk_toutes_tables("pk", "id")
    ...
    u_sql.formater_batrubtyp("t_originel_completee")
    u_sql.formater_champ("t_originel_completee", "date")
    ...
    u_sql.remplacer_null_par_vide("t_originel_completee")

    # 10) Initialisation de la table "t_lexique_cles"
    u_sql.supprimer_table("t_lexique_cles")
    u_sql.creer_lexique_cles("t_originel_completee")
    doublons = u_sql.lister_doublons("t_originel_completee", "cle")
    print(
